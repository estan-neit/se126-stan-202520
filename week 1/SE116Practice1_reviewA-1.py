...
def again(): #<--FUNCTION HEADER
    ...
    ans = input("\t\tWould you like to enter another temparture? [y/n]: ").lower()
...
def converter(f):
    #'f' is a parameter
    '''this function is passed an argument value of 'f' (a tempF), converts to tempC, and then returns to the conversion value'''
    c = (f - 32) * (5 / 9)
...
while answer == "y":
    ...
    tempC = converter(tempF) #(tempF - 32) * (5 / 9)
    ...
    answer = again() #return value will replace this function call abd store to 'answer'
...
avg_tempC = converter(tempF) #(avg_tempF - 32) * (5 / 9)
